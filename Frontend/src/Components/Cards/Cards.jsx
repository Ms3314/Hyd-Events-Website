...
const Cards = () => {
    ...
    useEffect(()=>{
        axios.get('http://localhost:3000/api/v1/user/event').then((data)=>{
            console.log(data.data.payload)
            setData1(data.data.payload)
        }).catch((error)=>{
...
const Cardcomponent = ({item}) => {
    ...
    return (
     <div className="bg-white rounded-lg w-[270px] shadow-md overflow-hidden flex flex-col 
                     hover:border-2 border-r-secondary borde    r-t-primary border-l-secondary border-b-secondary">
      
      <div className="h-64 relative">
             <img src={item.eventImage} alt={item.id} className="w-full h-full object-contain" />
        ...
        <div className="p-4">
            ...
            <div className='text-primary text-lg-center'>{(formattedDate)}</div>
         
            <div className="text-lg text-gray-600">{item.title} 
              <div className="text-sm text-gray-600">Conducted by : {item.organization.name}</div>
              {/* <div className="text-sm text-gray-600">{item.Timings}</div> */}
                ...
                <button className='mt-3 border-2 px-1 rounded-full' key={item.id} onClick={() => SpecificEventPage(item)}>
                 <svg  xmlns="http://www.w3.org/2000/svg" height="30px" viewBox="0 -960 960 960" width="20px" fill="#0000F5"><path d="m560-240-56-58 142-142H160v-80h486L504-662l56-58 240 240-240 240Z"/></svg>
                </button>
            </div>
